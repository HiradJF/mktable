...
class Table:
    ...
    def get_columns(self) -> list:
        """
        Transforms the rows and returnes the column
        """
        columns = []
    ...
    def render(self) -> list:
        ...
        def format_row(row: list) -> list:
            ...
            for i, row in enumerate(row):
                ...
                diff = max_column_widths[i] - len(row)
                ...
                formatted_cell = f"{v_sep_char} {row}{' '*diff} "
        ...
        horizontal_separator = (
            connection_point + (h_sep_char * border_len) + connection_point
        )
    ...
    def get_rows_lengths(self):
        """You can get every row's length with this method"""
        rows = self.rows
...
if __name__ == '__main__':
    t = Table(['First Name', 'Last Name', 'Age', 'Email'])
    t.add_row(['Hirad', 'Jahangirfard', '13', 'hiradjahangirfard@jmail.com'], ['Behrad', 'Jahangirfard'])
    t.add_row(['Taher', 'Jahangirfard', '48', 'jahangiri_it@yahoo.com'])
    print(t)